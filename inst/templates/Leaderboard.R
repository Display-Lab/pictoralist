...
make_plot <- function(plot_data) {
  plot_data %>% ggplot(aes(y=rank)) +
    geom_label(x=1, aes(y=rank, label=rank)) +
    geom_point(x=2, aes(y=rank, shape=delta_shape)) +
    geom_label(x=3, aes(y=rank, label=delta_label)) +
    geom_label(x=4, aes(y=rank, label=id)) +
    geom_label(x=5, aes(y=rank, label=rate_label)) +
    geom_label(x=6, aes(y=rank, label=count_label)) +
    scale_shape_manual(values=c("23"=23,"24"=24,"25"=25)) +
...
foo
#col_names <- c("RANK", "CLINIC NAME", "COUNSEL RATE", "PATIENTS")
make_plot(df)
